#!/usr/bin/env python3
"""
Camera to End-Effector Pose Publisher Node
Publishes the end-effector pose relative to the camera frame as PoseStamped message
"""
...
class CameraToEEPosePublisher(Node):
    def __init__(self):
        super().__init__('camera_to_ee_pose_publisher')
        ...
        self.get_logger().info(f'Camera to End-Effector Pose Publisher initialized')
        self.get_logger().info(f'Will publish transform from {self.camera_frame} to {self.ee_frame}')
        self.get_logger().info(f'Topic: /camera_to_ee_pose at {self.publish_rate} Hz')
    ...
    def start_publishing(self):
        ...
        self.publish_timer = self.create_timer(1.0 / self.publish_rate, self.publish_camera_to_ee_pose)
        self.get_logger().info(f'Started publishing camera-to-end-effector pose')
        
    def publish_camera_to_ee_pose(self):
        """Get transform and publish as PoseStamped"""
        try:
            # Look up transform from camera to end-effector
            ...
            if current_time - self._last_warning_time > 5.0:  # Log every 5 seconds
                self.get_logger().warn(
                    f'Could not get transform from {self.camera_frame} to {self.ee_frame}: {str(e)}'
                )
