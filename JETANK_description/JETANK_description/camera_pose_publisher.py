#!/usr/bin/env python3
"""
Camera Pose Publisher Node
Publishes the camera link pose as PoseStamped message
"""
...
class CameraPosePublisher(Node):
    def __init__(self):
        ...
        self.get_logger().info(f'Camera Pose Publisher initialized')
        self.get_logger().info(f'Will publish transform from {self.base_frame} to {self.camera_frame}')
        self.get_logger().info(f'Topic: /camera_pose at {self.publish_rate} Hz')
    ...
    def start_publishing(self):
        ...
        self.publish_timer = self.create_timer(1.0 / self.publish_rate, self.publish_camera_pose)
        self.get_logger().info(f'Started publishing camera pose')
        ...
        try:
            ...
            if current_time - self._last_warning_time > 5.0:  # Log every 5 seconds
                self.get_logger().warn(
                    f'Could not get transform from {self.base_frame} to {self.camera_frame}: {str(e)}'
                )
